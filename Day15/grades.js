...
/**
 * 
 * @returns {quiz} factory function that builds and returns quiz object
 */
function Grades() {
    ...
    function answerComparator(ans1, ans2) {
        ...
        for (let i = 0; i < ans1.length; i++) {
            for (let j = 0; j < ans2.length; j++) {
                if (ans1[i].qid == ans2[j].qid) {
                    if (ans1[i].ans == ans2[j].ans) {
                        total++;
                    }
                }
            }
    ...
    quiz.scoreStudent = function (sid) {
        ...
        for (let std of this.students) {
            if (std.sid == sid) {
                return answerComparator(std.answers, quiz.key);
            }
        }
        return "Wrong Id!";
    };
    ...
    quiz.getAverage = function () {
        ...
        let sum = 0;
        for (let std of this.students) {
            sum += this.scoreStudent(std.sid);
            console.log(this.scoreStudent(std.sid));
        }
