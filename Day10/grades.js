...
quiz.scoreStudent = function (sid) {
    //IMPLEMENT THIS
    let count = 0;
    for (let i = 0; i < quiz.students.length; i++) {
        if (quiz.students[i].sid == sid) {
            for (let j = 0; j < quiz.students[i].answers.length; j++) {
                for (let k = 0; k < quiz.students[i].answers.length; k++) {
                    if (quiz.key[j].qid == quiz.students[i].answers[k].qid) {
                        if (quiz.key[j].ans == quiz.students[i].answers[k].ans) {
                            count++;
                        }
                    }
                }
...
quiz.getAverage = function () {
    //IMPLEMENT THIS
    let sum = 0;
    for(let all of quiz.students){
        sum += quiz.scoreStudent(all.sid);
    }
